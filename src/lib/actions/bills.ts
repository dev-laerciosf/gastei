...
export async function getBills(month?: string) {
  ...
  const debtIncomes = await prisma.transaction.findMany({
    ...
    select: {
      id: true, description: true, amount: true, date: true, debtPaid: true, debtPersonName: true,
      repayments: { select: { id: true, amount: true, note: true, date: true }, orderBy: { date: "asc" as const } },
    },
    orderBy: { date: "desc" },
  });
